...
| `industry-rotation` | 看申万行业近期涨跌/波动的图表与读数——只观察、不算动量、不给交易信号 |
...
| 5. 汇率不写死 | 多币种标的的汇率必须 `fx.py` 取当日值，且只在计算链末端出现一次（见下） |

> 估值三档溯源（[观测]/[推算]/[估计] 标注）+ 横向对标必查由 L2 强制 enforce，不在 L1 重复。详见 [`docs/data-discipline.md`](docs/data-discipline.md#数字-sanity-check-详解)。

#### 多币种口径规则（2026-07-29 新增，源于一次三连错）

> **教训**：腾讯页写死 `1 RMB = 1.08 HKD`、小米页 1.087、美团页曾写 0.92（方向还反了）。查官方中间价——这三个值**在各自成文当天就已经错了**（2026-05-13 实为 1.1441、05-27 实为 1.1475），不是"期间汇率变动"。后果：港币计价 EPS 与全部四档锚点系统性低估 6-7%，且改动后数字仍自洽，L1 自检与肉眼都发现不了。同一根因在三页独立发生三次 → 缺的是机制不是自觉。

三条硬规则：

1. **建模用报表货币，展示用交易货币**。港股里腾讯/阿里/美团/小米/网易均以人民币记账 → 营收、利润、EPS、DCF、SOTP **全程用人民币**算；四档锚点**用港币**展示（与港股通报价、下单界面一致）。港交所、长和这类本就港币记账的标的，端到端零换算。
2. **汇率只在计算链末端出现一次**：`人民币每股合理价值 × 当日汇率 = 港币锚点`。禁止在远期 EPS 推导、SOTP 分部折算等中间环节反复换算——FX 出现 N 次就有 N 个衰减点。
3. **汇率必须"取"不能"写"**：唯一来源 `skills/_shared/marketdata/fx.py`（外管局/中行官方中间价，方向写死在函数名里：`hkd_per_cny()` / `cny_per_hkd()`）。**禁止在 wiki 正文硬编码汇率常量**；确需在正文写出汇率值时必须同行标注日期。由 `lint-fx-constant.py` 机械兜底（warn-only）。

**例外：历史 PE/PB 分位**。这是唯一躲不掉多期汇率的地方（每一年要用当年汇率）。**不要用"港币价格 ÷ 自算港币 EPS"回溯**——直接取数据商现成的 PE 时间序列（对齐 A 股用 baostock `peTTM` 的既有做法，避免"分位反推中位"陷阱）。

**结果层提醒**：港股通是**港币报价、人民币结算**（中国结算轧差换汇，买卖各一次），故汇率是**组合层的独立损益项**，应单列跟踪，不要让它散落在各个个股模型里。
...
| 写港股（或任何报表货币 ≠ 交易货币的标的）的每股价格 / EPS / 四档锚点 | 用 `fx.py` 取当日中间价，禁止硬编码；换算只在末端做一次 |
