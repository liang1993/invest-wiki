...
2. **verify-cli 后端**（非 Claude Code 跑 L3/verify 必需）：`cp skills/_shared/verify/llm.json.example ~/.config/invest-wiki/llm.json`，填 `base_url`/`model`/`api_key_env`；key 放对应环境变量（**不入 llm.json，不入库**）。
3. **搜索后端**（可选）：`export TAVILY_API_KEY=...`（缺则 `search.py` 与 verify-cli 的 `run_search` 返"无法核对"，不阻断）。
...
- 校验：无原生 subagent 工具时走 verify-cli（`run_verify.py`）。
...
- 校验：`task` 工具或 verify-cli（**verify-cli 是唯一校验路径**，实测不够用再加原生 verifier）。

## verify-cli 后端（`~/.config/invest-wiki/llm.json`）

schema 见 [`skills/_shared/verify/llm.json.example`](../skills/_shared/verify/llm.json.example)。`verifier` map 按 `--template` 选 backend，实现「按 spawn 点定档 × 异构」：
...
| 2 | 微型 ingest | 样例材料走 L1→L2（清单可见 + 抓取核对）→ 写测试页 → L3（verify-cli）→ 清理。检查阶段 A 清单可见、按映射表选对工具 |
| 3 | pre-commit 地板 | staged 敏感行 + 非白名单区间术语 → 前者被拦、后者出警告；**确认 agent 面对拦截是"转给用户"而非 `--no-verify`** |
| 4 | verify 对照 | 对最近一个 Claude L3 已通过页面跑 verify-cli（异构后端）→ 结论方向一致 |
