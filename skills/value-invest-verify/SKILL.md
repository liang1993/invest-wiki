...
### Step 2: 调起独立校验员（原生 subagent 优先）

当前环境能启动零历史的原生 subagent，并提供读文件、搜索和原文抓取能力时，直接使用原生工具。Codex 使用零历史 fork（如 `fork_turns="none"`）；Claude Code 使用 `Agent` 工具，`subagent_type=general-purpose`。prompt 严格按照 `prompt-template.md` 的模板（见 [`prompt-template.md`](prompt-template.md)）填充：
...
> **verify-cli fallback**：仅当当前环境没有满足要求的原生 subagent，或用户明确要求异构模型 / CLI 退出码时，才运行 `python3 skills/_shared/verify/run_verify.py --template value-invest --file <个股wiki>`（渲染同一份 `prompt-template.md`）。调用前检查 `~/.config/invest-wiki/llm.json`；缺配置但原生 subagent 可用时直接走原生路径，不要先调用一次已知会失败的 verify-cli。
...
**实现**：优先用原生工具调起**第三个零上下文 subagent**（仲裁员 / arbiter），prompt 用 [`arbiter-prompt.md`](arbiter-prompt.md) 模板。没有合格原生 subagent，或明确需要异构模型 / CLI 退出码时，才运行 `python3 skills/_shared/verify/run_verify.py --template arbiter --diff <分歧描述文件>`（调用前检查本机配置；arbiter 模式机械排除读取 `wiki/**`，把 `arbiter-prompt.md` "严禁读 wiki" 的盲化从指令级升为代码级）。
